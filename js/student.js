...
function showInventory(username) {
    ...
    // Function to update the inventory in local storage
    const updateInventory = (newInventory) => {
        localStorage.setItem(`${username}_inventory`, JSON.stringify(newInventory));
        showInventory(username);  // Re-render the inventory
    };

    inventoryList.innerHTML = userSpecificInventory.map((item, index) => `
        <li>
            ...
            <button onclick="removeItem(${index}, '${username}')">Remove</button>
        </li>
    `).join('');
}

// Function to remove an item from the user's inventory
function removeItem(index, username) {
    const userSpecificInventory = JSON.parse(localStorage.getItem(`${username}_inventory`)) || [];
    userSpecificInventory.splice(index, 1);  // Remove the item at the specified index
    localStorage.setItem(`${username}_inventory`, JSON.stringify(userSpecificInventory));
    showInventory(username);  // Re-render the inventory after removal
}
